...
export default class Cli {
  ...
  /**
   * Lists the installed generators. The packages with the justo-generator keyword.
   */
  static listInstalledGenerators() {
    var ls, gens = [];
    const table = require("text-table");

    //(1) get packages
    ls = child_process.spawnSync("npm", ["ls", "-g", "--json", "--long"]);
    ls = JSON.parse(ls.stdout.toString()).dependencies;

    //(2) filter generators: justo-generator
    for (let pkgName in ls) {
      let pkg = ls[pkgName];
  ...
  /**
   * Lists the installed modules. The packages with the justo-module keyword.
   */
  static listInstalledModules() {
    var ls, mods = [];
    const table = require("text-table");
