...
function register(...args) {
  ...
  if (item instanceof Function) {
    if (item.__task__) task = item;   //register task
    else task = workflow(opts, item); //register workflow using flow function
  } else {  //register() has been invoked with an array of strings
    task = workflow(opts, function(params) {
      //previously, checkWorkflows() has been run and this garantees the workflows to exist
      for (let name of item) {
        registers[name].task(name, ...params);
      }
    });

    task.checkWorkflows = function() {
      for (let name of item) {
        if (!registers.exists(name)) throw new Error(`The '${name}' workflow is not registered.`);
      }
    };
  }
...
export function list(file) {
  ...
  for (let name of Object.keys(registers).sort()) {
    ...
    tbl.push([reg.name, reg.description]);
  }
