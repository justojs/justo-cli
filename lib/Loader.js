...
export default class Loader {
   ...
   * @param opts:object The runner options: only (boolean).
   */
  static loadJustoJs(file, opts = {only: false}) {
    var justo, autom;

    //(1) initialize justo
    justo = Loader.importJusto();
    justo.initialize(Object.assign({only: opts.only}, JustoJson.config));
